...
Image *image_read(char *filename)
{
   ...
   for (int i = 0; i < rows * cols; i++){
       src->data[i].rgb[0] = (float)(image[i].r) / 255.0;
       src->data[i].rgb[1] = (float)(image[i].g) / 255.0;
       src->data[i].rgb[2] = (float)(image[i].b) / 255.0;
   }
   free(image);
   return src;
}
...
int image_write(Image *src, char *filename)
{
    ...
    for (int i = 0; i < src->rows * src->cols; i++){
       image[i].r = (unsigned char)trunc(src->data[i].rgb[0] * 255.0);
       image[i].g = (unsigned char)trunc(src->data[i].rgb[1] * 255.0);
       image[i].b = (unsigned char)trunc(src->data[i].rgb[2] * 255.0);
   }
